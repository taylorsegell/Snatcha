//Constants
const LOADING_ICON = $(document).find("#snatcha-svg-icon").html();
...
const bounceCss = `@keyframes rotate{from{transform:rotate(0)}to{transform:rotate(360deg)}}.snatcha-icon{animation:2s linear infinite rotate}`
...
const downloadIcon = function (text = "", downloadAbleName = "", extension = "svg", drc = false) {
    ...
    newAnchorElement.setAttribute("download", (downloadAbleName + `.${extension}`) ?? "snatcha.svg");
...
const downloadJson = function (data = {}, downloadAbleName = "", extension = "json") {
    ...
    link.href = url;
    link.download = (downloadAbleName + "." + extension) ?? "snatcha.json";
    document.body.appendChild(link);
    link.click();
...
const activateButtons = function () {
    let snatchaPremiumElements = $("#fi-premium-download-buttons");
    snatchaPremiumElements.find(".icon--premium").hide();
    snatchaPremiumElements.find(".modal-body .hr:first").prevUntil("p.font-md.bold").remove();
